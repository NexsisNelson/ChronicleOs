...
if (-not (Test-Path "apps/agents/.env")) {
  ...
  Write-Host "Created apps/agents/.env from the example template. Update it before deploying."
}

if (-not (Test-Path "apps/dashboard/.env.local")) {
  Copy-Item "apps/dashboard/.env.example" "apps/dashboard/.env.local"
  Write-Host "Created apps/dashboard/.env.local from the example template. Update it before deploying."
}
